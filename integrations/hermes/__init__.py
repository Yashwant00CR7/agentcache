"""
agentcache memory provider for Hermes Agent.
...
# os.environ in the Hermes CLI process, finds AGENTCACHE_URL /
# AGENTCACHE_SECRET unset, and reports the plugin as "Missing" even
# though the service is healthy and live sessions can use it.
#
...
def _preload_agentcache_dotenv() -> None:
    candidates: list[Path] = []
    home = os.environ.get("HOME")
    if home:
        candidates.append(Path(home) / ".agentcache" / ".env")
    xdg_config = os.environ.get("XDG_CONFIG_HOME")
    if xdg_config:
        candidates.append(Path(xdg_config) / "agentcache" / ".env")
    for path in candidates:
        try:
            ...
            continue
    # Guarantee AGENTCACHE_URL is set so `hermes memory status` never
    # reports it as Missing when the server runs on the default
    # localhost:3111 (or via systemd with the URL line commented out in
    # ~/.agentcache/.env because it matches the default).
    os.environ.setdefault("AGENTCACHE_URL", DEFAULT_BASE_URL)


_preload_agentcache_dotenv()
...
def _plaintext_bearer_auth_message(base: str) -> str:
    return f"agentcache: AGENTCACHE_SECRET is configured for plaintext HTTP to {base}. Bearer tokens and memory payloads can be observed on the network; use HTTPS or an SSH tunnel."
...
def _api(base: str, path: str, body: dict | None = None, method: str = "POST", secret: str = "") -> dict | None:
    ...
    auth = secret or os.environ.get("AGENTCACHE_SECRET", "")
...
class AgentCacheProvider(MemoryProvider):
    ...
    def is_available(self) -> bool:
        ...
        base = os.environ.get("AGENTCACHE_URL", DEFAULT_BASE_URL)
        return _validate_url(base)

    def initialize(self, session_id: str, **kwargs: Any) -> None:
        self._base = os.environ.get("AGENTCACHE_URL", DEFAULT_BASE_URL)
    ...
    def get_config_schema(self) -> list[dict]:
        return [
            {
                "key": "url",
                "description": "agentcache server URL",
                "default": DEFAULT_BASE_URL,
                "env_var": "AGENTCACHE_URL",
            },
            {
                "key": "secret",
                "description": "agentcache auth secret (optional)",
                "secret": True,
                "required": False,
                "env_var": "AGENTCACHE_SECRET",
            },
        ]

    def save_config(self, values: dict, hermes_home: str) -> None:
        config_path = Path(hermes_home) / "agentcache.json"
    ...
    def get_tool_schemas(self) -> list[dict]:
        return [
            {
                "name": "memory_recall",
                "description": "Search agentcache for past observations by keyword",
                "parameters": {
                    "type": "object",
                    "properties": {
...
def register(ctx: Any) -> None:
    ctx.register_memory_provider(AgentCacheProvider())
