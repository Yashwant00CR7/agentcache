"""Legacy module — remaining concerns pending extraction. Do not add new code here."""
...
# =====================================================================
# Search state is now owned by SearchService (core/search_service.py).
# _search_service is set by set_search_service() during app init.
...
# KV is imported from core.kv_scopes above — see core/kv_scopes.py
...
# IndexPersistence is imported from core.search_service above.
...
def vector_index_add_guarded(
    ...
    if _search_service is None:
        return False
    ep = _search_service.embedding_provider
    vi = _search_service.vector
    if not vi or not ep:
        return False
    try:
...
def observe(kv: StateKV, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _search_service:
        _search_service.schedule_persist()
...
def remember(kv: StateKV, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _search_service:
        try:
            _search_service.bm25.add(memory_to_observation(new_mem))
        except Exception as ex:
...
def evolve_memory(kv: StateKV, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _search_service:
        try:
            _search_service.bm25.add(memory_to_observation(new_mem))
            _search_service.bm25.remove(existing["id"])
        except Exception:
            pass
    ...
    if _search_service and _search_service.vector:
        _search_service.vector.remove(existing["id"])
...
def auto_forget(kv: StateKV, dry_run: bool = False) -> Dict[str, Any]:
    ...
    if not dry_run:
        ...
        for sid, obs_id in evicted_observations:
            ...
            if _search_service:
                _search_service.remove(base_oid)
                _search_service.remove(f"{base_oid}:raw")
...
def health_check(kv: StateKV) -> Dict[str, Any]:
    ...
    bm25_index_size = 0
    try:
        bm25_index_size = _search_service.bm25.size if _search_service else 0
    except Exception:
        pass
    ...
    try:
        if _search_service and _search_service.vector:
            vector_index_size = _search_service.vector.size
    except Exception:
        pass
...
def consolidate(
    ...
    for concept, obs_group in sorted_groups:
        ...
        try:
            ...
            if existing_match:
                ...
                if _search_service:
                    try:
                        _search_service.bm25.add(memory_to_observation(evolved))
                        if existing_match:
                            _search_service.bm25.remove(existing_match["id"])
                    except Exception:
                        pass
                ...
                if _search_service and _search_service.vector and existing_match:
                    try:
                        _search_service.vector.remove(existing_match["id"])
                    except Exception:
                        pass
                ...
                if _search_service:
                    try:
                        _search_service.bm25.add(memory_to_observation(memory))
                    except Exception:
                        pass
    ...
    if _search_service and consolidated_count > 0:
        _search_service.schedule_persist()
