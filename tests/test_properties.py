...
def _fresh_store(kv):
    """Create an ObservationStore wrapping the given KV."""
    from agentcache.core.observation_store import ObservationStore

    return ObservationStore(kv=kv)
...
def test_property_1_pair_isolation(path1, agent1, path2, agent2, text):
    ...
    kv = _fresh_kv()
    store = _fresh_store(kv)
    store.ingest(
        {"folderPath": path1, "agentId": agent1, "text": text, "timestamp": _now()}
    )
...
# No stored obs text contains raw secrets after ingest().
# ---------------------------------------------------------------------------
...
# timeline() always returns results sorted newest-first.
# ---------------------------------------------------------------------------
...
def test_property_5_timeline_ordering(path, agent, n):
    ...
    results = store.timeline(limit=100, folder_path=path, agent_id=agent)
...
def test_property_6_forget_completeness(path, agent, texts):
    ...
    store.forget({"folderPath": path, "agentId": agent})
    normalized_path = path.replace("\\", "/").strip("/")
    normalized_agent = agent.strip()
...
def test_property_7_memory_version_uniqueness(base_content, n_variants):
    from agentcache.core.kv_scopes import KV
    from agentcache.legacy import remember
